...
class RewardSystem:
    def __init__(self):
        self.rewards = {
            ...
            "stickers": ["Spidey And Friends Stickers", "Dinosaur Stickers", "Hearts and Stars Stickers, Paw Patrol Pup Stickers"],
            "playtime": ["5 minutes extra screentime", "10 minutes extra physical games", "15 minutes extra at Ninja U", "10 minutes of bubble time"],
...
def build_educational_tree():
    ...
    mathematical_understanding = EducationalObjective("Numbers and Counting", "Cognitive", 3)
...
def randomize_schedule(schedule, root):
    ...
    activities = [
        ...
        ("Monday", "Numbers and Counting", "Count with flashcards."),
        ("Tuesday", "Social and Emotional Development", "Head to ARL and volunteer with animals"),
        ("Wednesday", "Creative Arts and Expression", "Identify different shapes and talk about differences."),
        ("Friday", "Creative Arts and Expression", "Paint by numbers with Mom/Dad"),
...
def display_archive_gui():
    archive_window = tk.Toplevel(app)
    archive_window.title("Activity Archive")
    archive_text = tk.Text(archive_window, wrap="word")
    archive_text.pack(expand=True, fill="both")

    for day, activities in archive.archive.items():
        archive_text.insert("end", f"Activities for {day}:\n")
        for activity in activities:
            archive_text.insert("end", f" - {activity['objective']}: {activity['activity']} rewarded with {activity['reward']}\n")
    archive_text.config(state="disabled")
...
button_view_archive = tk.Button(app, text="View Archive", command=display_archive_gui)
button_view_archive.pack(pady=(10, 10))
