...
def build_educational_tree():
    ...
    language_literacy.add_sub_objective(EducationalObjective("Phonics and Reading", "Cognitive", 3, ["Sound out letters", "Read simple words"]))
    ...
    mathematical_understanding.add_sub_objective(EducationalObjective("Patterns and Sequences", "Cognitive", 3, ["Recognize patterns", "Create sequences"]))
    ...
    motor_skills.add_sub_objective(EducationalObjective("Gross Motor Skills", "Physical", 3, ["Jumping over obstacles", "Balancing"]))
...
def populate_schedule(schedule):
    ...
    schedule.add_activity("Monday", root.find_objective("Social and Emotional Development"), "Identify emotions with flash cards")
    schedule.add_activity("Tuesday", root.find_objective("Shapes and Colors"), "Find and name circles around the room.")
    schedule.add_activity("Tuesday", root.find_objective("Drawing and Painting"), "Get a small canvas and acrylic paints, put on Bob Ross and paint")
    schedule.add_activity("Wednesday", root.find_objective("Gross Motor Skills"), "Jumping over obstacles or Balancing")
    schedule.add_activity("Wednesday", root.find_objective("Understanding Emotions"), "Watch an educational video on empathy and talk about different emotions")
    schedule.add_activity("Thursday", root.find_objective("Fine Motor Skills"), "Pincer grasp activities (picking up loose change")
    schedule.add_activity("Thursday", root.find_objective("Phonics and Reading"), "Head to the Local Library and find new books to read/contribute to StoryTime")
    schedule.add_activity("Friday", root.find_objective("Science and Enviornmental Awareness"),"Take a trip to the Botanical gardens or help the neighbors in theirs")
...
def checkbox_callback(var, objective):
    if var.get():
        ...
        reward_choice = reward_options[0] if reward_options else "a pat on the back"
